...
class ExcelPage
{
	...
	private function getRecentTournamentResults()
	{
		$db = Database::getConnection()->getPDO();
		
		$tournamentCount = Config::getConfig()->getValue('excel_nr_recent_tournaments');
		
		$tmpRecentTournaments = $db->query('SELECT DISTINCT(t.tournament_id), t.tournament_date ' .
										'FROM results r ' .
										'LEFT JOIN tournaments t ON r.tournament_id=t.tournament_id ' .
										'ORDER BY t.tournament_date DESC '.
										'LIMIT ' . $tournamentCount);
		
		$recentTournaments = array();
		$recentTournamentIDs = array();
		
		foreach ($tmpRecentTournaments as $rt)
		{
			$recentTournaments[] = $rt;
			$recentTournamentIDs[] = $rt->tournament_id;
		}
		
		$tmpRecentPoints = $db->query('SELECT player_id, points ' .
									'FROM results ' .
									'WHERE tournament_id IN (' . implode(',', $recentTournamentIDs) . ')');
		
		$recentPoints = array();
		foreach ($tmpRecentPoints as $rp)
		{
			$recentPoints[] = $rp;
		}
	}
